...
app = Flask(__name__)
es_connector = DatabaseConnector(environment="prod")
...
def search_books():
    ...
    response = es.search(index="books_data_bis", body=search_query)
...
def register():
    ...
    if any(len(field) > 50 for field in [username, email, password, confirm_password]):
            return jsonify({"error": "Fields must not exceed 50 characters"}), 400
